...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  FLOATUNION_t deltab;	//Tempo entre calculos de angulos do gyro
  ...
  deltab.number = dt;
  accxb.number = accel_angle_x;
  accyb.number = accel_angle_y;
  acczb.number = accel_angle_z;

  gyxb.number = unfiltered_gyro_angle_x;
  gyyb.number = unfiltered_gyro_angle_y;
  gyzb.number = unfiltered_gyro_angle_z;

  pitchb.number = angle_x;
  rollb.number = angle_y;
  yawb.number = angle_z;

  // Send the data to the serial port
  Serial.write('#');              //Delta T
    Serial.write('#'); 
      Serial.write('#'); 
        Serial.write('#'); 
...
//  
//  Serial.write(deltab.bytes, 4);
//
//  Serial.write(accxb.bytes, 4);
//  Serial.write(accyb.bytes, 4);
//  Serial.write(acczb.bytes, 4);
//  
//  Serial.write(gyxb.bytes, 4);
//  Serial.write(gyyb.bytes, 4);
//  Serial.write(gyzb.bytes, 4);
//
//  Serial.write(pitchb.bytes, 4);
//  Serial.write(rollb.bytes, 4);
//  Serial.write(rollb.bytes, 4);
